...
> **Sync rule:** When any change is made to this file, the agent MUST apply the same change to `CLAUDE.md`, `AGENTS.md`, and `.cursorrules` so all agent instruction files stay in sync. Edit any of the four → update the others. The four files must always have identical content (only the title line differs).
