...
| 选项 | 数据范围 | 与哪些图表对齐 |
|------|---------|--------------|
| 全部 | X 坐标不限 | Hexbin 全场热力图的汇总视图 |
| 左半场 | `X < 0` 的投篮 | Hexbin 左半场、Time-FG% 左半场折线 |
| 右半场 | `X > 0` 的投篮 | Hexbin 右半场、Time-FG% 右半场折线 |

切换后桑基图从 JSON 数据中按 `X` 坐标正负号过滤，重新聚合四层数据。预处理 JSON 需在每条聚合记录中增加 `court_side` 字段。
